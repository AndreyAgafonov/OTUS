...
if [ -f "$MKEYNAME" ] ;
        ...
        openssl req -new -key $MKEYNAME -x509 -out $MCERTNAME -subj '/C=RU/ST=Moscow/L=Moscow/O=OTUS/OU=LinuxAdmin/CN=bserver'
fi
if [ -f "$FDKEYNAME" ] ;
        then
        ...
        openssl req -new -key $FDKEYNAME -x509 -out $FDCERTNAME -subj '/C=RU/ST=Moscow/L=Moscow/O=OTUS/OU=LinuxAdmin/CN=bserver'
fi
